...
HTML;
    ...
    private static  function get_field_by_type($type, $name, $placeholder, $options = [], $requried = null, $mimes = null)
    {
        ...
        switch ($type) {
            ...
            case('textarea'):
                $required_markup = !empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="form-group textarea"><label for="' . $name . '">' . __($placeholder) . '</label> <textarea name="' . $name . '" id="' . $name . '" cols="30" rows="3" class="form-control" placeholder="' . __($placeholder) . '" ' . $required_markup . '></textarea></div>';
                break;
            case('file'):
                $required_markup = !empty($requried) ? $required_markup_html : '';
