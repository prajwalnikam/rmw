...
class JobProposal extends Model
{
    ...
    protected $fillable = ['job_id','freelancer_id','client_id','amount','duration','revision','cover_letter','attachment','status','is_hired','is_short_listed','is_interview_take','load_from',
        'is_synced','attached_resource_ids'];

    protected $casts = ['status'=>'integer','is_hired'=>'integer','is_short_listed'=>'integer','is_interview_take'=>'integer','is_view'=>'integer','is_rejected'=>'integer','attached_resource_ids' => 'array'];
    ...
    {
        // Assuming your Resource model exists and you want to directly query them.
        // This is a "manual" relationship, not a standard Eloquent one,
        // because the IDs are stored in a JSON column, not a pivot table.
        return $this->hasMany(Resource::class, 'id', function($query) {
            // Get the IDs from the JSON column
            $resourceIds = $this->attached_resource_ids ?? [];
            return $query->whereIn('id', $resourceIds);
        });

        // A more direct way to get them would be:
        // public function getResourcesAttribute()
        // {
        //     if (empty($this->attached_resource_ids)) {
        //         return collect();
        //     }
        //     return Resource::whereIn('id', $this->attached_resource_ids)->get();
        // }
    }
}
