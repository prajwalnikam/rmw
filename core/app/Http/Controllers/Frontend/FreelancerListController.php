...
class FreelancerListController extends Controller
{
    ...
    public function talents(Request $request)
    {
        ...
        $resources = Resource::with('user')
            ...
            ->select(['id', 'title', 'description', 'status', 'role', 'experience', 'monthly_salary', 'hourly_salary', 'user_id'])
            ->paginate(9);

            if($request->ajax()){
                return view('frontend.pages.talent.resources', compact('resources'))->render();
            }


            return view('frontend.pages.talent.resources', compact('resources'));
    ...
    public function pagination(Request $request)
    {
        ...
        if ($request->ajax()) {
            $resources = Resource::with('user')
                ...
                ->select(['id', 'title', 'description', 'status', 'role', 'experience', 'monthly_salary', 'hourly_salary', 'user_id'])
                ->paginate(9);
            return view('frontend.pages.talent.resources-pagination', compact('resources'))->render();
        }
    }
    ...
    public function show($id)
    {
        ...
        return view('frontend.pages.talent.resource-view', compact('resource'));
    }
    ...
    public function contact(Request $request, $id)
{
    ...
    $resource = Resource::with('user')->findOrFail($id);
    $resourceOwnerEmail = $resource->user->email;
    $resourceName = $resource->title;

    $userEmail = auth()->user()->email;
    $userMessage = $request->input('message');

    Mail::send('mail.resource-contact', [
        'email' => $userEmail,
        'userMessage' => $userMessage,
        'resourceName' => $resourceName
    ], function ($message) use ($resourceOwnerEmail) {
        $message->to($resourceOwnerEmail)
            ->subject('Contact Request from Resource Page')
            ->from(config('mail.from.address'), config('mail.from.name'))
            ->replyTo($resourceOwnerEmail);
    });

    return redirect()->route('resources.show', $id)->with('success', 'Email sent successfully.');
}
