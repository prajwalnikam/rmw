...
use App\Http\Controllers\Frontend\Client\ProjectRequirementController;
...
// use App\Http\Controllers\Frontend\Client\ProjectRequirementController;
// use App\Http\Controllers\Frontend\Client\JobProjectController;
// use App\Http\Controllers\Frontend\Client\JobRequirementController;
use App\Http\Controllers\Frontend\Client\JobProjectController;
...
Route::group(['prefix'=>'client','as'=>'client.'],function() {
    ...
    Route::group(['middleware'=>['auth','userEmailVerify','Google2FA','globalVariable', 'maintains_mode','setlang']],function(){
        ...
        Route::post('proposal/{id}/contact', [JobController::class, 'contact'])->name('proposal.contact');
        // orders
    ...
    Route::get('/client/match-resources', [JobController::class, 'matchResources'])->name('client.match.resources');
Route::post('/client/match-resources', [JobController::class, 'getMatchedResources'])->name('client.get.matched.resources');
...
Route::group(['prefix' => 'client', 'as' => 'client.'], function () {
    Route::post('project/requirement/store', [JobProjectController::class, 'store'])->name('project.requirement.store');
});


Route::prefix('client')-> group(function () {
    Route::get('projects',[JobProjectController::class, 'index'])->name('client.projects.index');
    Route::get('projects/create', [JobProjectController::class, 'create'])->name('client.projects.create');
    Route:: post('projects', [JobProjectController::class, 'store'])->name('client.projects.store');

    // Route:: get('projects/{project}/job-requirements/create', [JobRequirementController::class, 'create'])->name('client.job-requirements.create');
    // Route:: post('projects/{project}/job-requirements', [JobRequirementController::class, 'store'])->name('client.job-requirements.store');
});
