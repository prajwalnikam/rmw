...
use App\Http\Controllers\Frontend\Freelancer\ResourceController;
    ...
    Route::group(['middleware' => ['auth', 'userEmailVerify', 'Google2FA', 'globalVariable', 'maintains_mode', 'setlang']], function () {
        ...
        Route::controller(ResourceController::class)->group(function () {
            Route::group(['prefix' => 'resource'], function () {
                Route::post('import','import')->name('resources.import');

                Route::get('all', 'index')->name('resource.index');
                Route::post('delete', 'destroy')->name('resource.delete');
                Route::post('update', 'update')->name('resource.update');
                Route::get('create', 'add')->name('resources.add');
                Route::post('store', 'store')->name('resource.store');
                Route::post('update-status', 'updateStatus')->name('resource.updateStatus');
            });
        });
        // Route::post('switch-profile', 'switch_profile')->name('switch.profile');
        // Route::controller(ResourceController::class)->prefix('resource')->group(fn() => Route::get('create', 'add')->name('freelancer.resources.add'));
        // Route::controller(ResourceController::class)->prefix('resource')->group(fn() => Route::post('store', 'store')->name('freelancer.resources.store'));
        // Route::controller(ResourceController::class)->prefix('resource')->group(fn() => Route::get('all', 'index')->name('freelancer.resources.index'));
        // Route::controller(ResourceController::class)->prefix('resource')->group(fn() => Route::post('update-status', 'updateStatus')->name('freelancer.resources.updateStatus'));
        // Route::controller(ResourceController::class)->prefix('resource')->group(fn() => Route::get('edit/{id}', 'edit')->name('freelancer.resources.edit'));
        // Route::controller(ResourceController::class)->prefix('resource')->group(fn() => Route::put('update/{id}', 'update')->name('freelancer.resources.update'));
        // Route::controller(ResourceController::class)->prefix('resource')->group(fn() => Route::post('delete', 'destroy')->name('freelancer.resource.delete'));
