...
class CategoryJobOne extends PageBuilderBase
{
    ...
    public function frontend_render()
    {
        ...
        $job_categories = Category::select('id','category','slug','image')
            ->where('status',1)
            ->whereHas('jobs')
            ->with('jobs')
            ->withCount('jobs')
        ...
        return  $this->renderBlade('categories.category-jobs-one',compact(['title','items','slider_button_text','padding_top','padding_bottom','section_bg','job_categories']));
    }
