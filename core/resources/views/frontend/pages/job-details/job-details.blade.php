                                        ...
                                        <div class="jobFilter-wrapper-item-form custom-form">
                                            ...
                                            <form action="{{ route('job.proposal.send') }}" method="post"
                                                ...
                                                <div class="single-flex-input">
                                                    ...
                                                    {{-- Show resources as cards --}}

<div class="container py-4">
    <h4 class="mb-3">{{ __('Select Resources for this Proposal') }}</h4>
    @if(isset($resources) && count($resources) > 0) {{-- Assuming $myResources is passed to the view --}}
        <div class="row">
                        ...
                        <div class="card-body">
                            <div class="form-check mb-2">
                                <input class="form-check-input resource-checkbox" type="checkbox"
                                       ...
                                       name="selected_resources[]"> {{-- THIS IS CRUCIAL --}}
                            ...
                            {{-- Display other fields from your Resource model --}}
                            <ul class="list-unstyled mt-3">
                                @if($resource->status)
                                    <li><strong>Status:</strong> <span class="badge {{ $resource->status == 'available' ? 'bg-success' : 'bg-warning' }}">{{ ucfirst($resource->status) }}</span></li>
                                @endif
                                @if($resource->role)
                                    <li><strong>Role:</strong> {{ $resource->role }}</li>
                                @endif
                                @if($resource->experience)
                                    <li><strong>Experience:</strong> {{ $resource->experience }} years</li>
                                @endif
                                @if($resource->monthly_salary)
                                    <li><strong>Monthly Salary:</strong> ${{ number_format($resource->monthly_salary, 2) }}</li>
                                @endif
                                @if($resource->hourly_salary)
                                    <li><strong>Hourly Salary:</strong> ${{ number_format($resource->hourly_salary, 2) }} / hour</li>
                                @endif
                            </ul>
                        </div>
                    </div>
    ...
    @else
        <div class="alert alert-info" role="alert">
            {{ __('You have no resources created yet. Please create resources to attach them to proposals.') }}
        </div>
    @endif
</div>

<div class="single-input">
    <label class="label-title"> {{ __('Number of Resources Selected') }} </label>
    <input type="number" name="revision" id="revision" class="form--control" readonly
        placeholder="{{ __('Number of selected resources') }}">
</div>
                                                ...
                                                {{-- <div class="single-input">
                                                    <label class="label-title"> {{ __('No. of resource available for job') }} </label>
                                                    <input type="number" name="revision" id="revision"
                                                        class="form--control"
                                                        placeholder="{{ __('') }}" onkeypress="inpNum(event)">
                                                </div> --}}
                                                

                                                
                                                {{-- <div class="single-input">
                                                    <label class="label-title"> {{ __('Details about resources') }} </label>
                                                    <textarea name="cover_letter" id="cover_letter" class="form-message" rows="3"
                                                        placeholder="{{ __('Write your Details minimum 10 characters...') }}"></textarea>
                                                </div> --}}
