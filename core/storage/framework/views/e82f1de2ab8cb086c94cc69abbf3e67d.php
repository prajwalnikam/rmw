...
(function($){
    ...
    $(document).ready(function(){
        ...
        $('#title').on('input', function(){
            ...
            if(length < minLen){
                $('#job_title_char_length_check').html(`<p class="text-danger"><?php echo e(__('Length is short, minimum')); ?> ${minLen} <?php echo e(__('required')); ?>.</p>`);
            } else if(length > maxLen){
                $('#job_title_char_length_check').html(`<p class="text-danger"><?php echo e(__('Length is not valid, maximum')); ?> ${maxLen} <?php echo e(__('allowed')); ?>.</p>`);
            } else {
                $('#job_title_char_length_check').html(`<p class="text-success"><?php echo e(__('Length is valid')); ?></p>`);
            }
        });
        ...
        $(document).on('click','#confirm_create_job',function(){
            ...
            if(!type || !budget || !skills){
                toastr_warning_js(<?php echo json_encode(__('Except attachment all fields required !'), 15, 512) ?>);
                return false;
            } else {
    ...
    $(document).on("click", "#next", function (e){
        ...
        if (current === 0) {
            ...
            if (!project_name || !project_description || project_name.length < 5 || project_description.length < 10) {
                toastr_warning_js(<?php echo json_encode(__('Please fill all fields and ensure minimum lengths.'), 15, 512) ?>);
                return;
            }
        }
        ...
        if (current === 2) {
            ...
            if (!category || !subcategory || !title || !description || !level || !duration ||
                ...
                toastr_warning_js(<?php echo json_encode(__('Please fill all fields and ensure valid lengths.'), 15, 512) ?>);
                return;
            }
...
<?php /**PATH /home/piks/Documents/rmwork-zv/core/resources/views/frontend/user/client/job/create/create-job-js.blade.php ENDPATH**/ ?>